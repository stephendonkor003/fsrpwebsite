        ...
        @forelse($events as $event)
            <article class="spotlight-event">
                <a class="spotlight-image" href="{{ route('events.show', [$locale, $event->slug]) }}">
                    <img src="{{ $event->image ?: asset('images/seed-investment-summit/seed-investment-summit-2026.jpeg') }}" alt="{{ $event->translate('title') }}" loading="lazy">
                ...
                <div class="spotlight-copy">
                    <div class="card-meta"><span class="tag tag-green">{{ __('ui.events.modes.'.$event->mode) }}</span><span class="partner-label">{{ __('portal.current_event') }}</span></div>
