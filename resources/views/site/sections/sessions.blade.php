    ...
    <div class="container">
        ...
        <div class="day-grid">
            @foreach($sessions as $session)
                @php
                    $isCaadpSession = $session->event?->slug === '22nd-caadp-partnership-platform';
                    $caadpImage = asset('images/caadp/caadp-partnership-'.(($loop->index % 4) + 1).'.jpeg');
                @endphp
                <article @class(['day-card', 'day-card-visual' => $isCaadpSession])>
                    @if($isCaadpSession)<img class="day-card-background" src="{{ $caadpImage }}" alt="" loading="lazy">@endif
